...
<div class="d-flex justify-content-center align-items-center" style="margin: 15px">
    <a href="{{ route('admins.edit',$car->id) }}" class="btn btn-warning" style="margin: 15px">Edit</a>
    <form style="display: inline" method="POST" action="{{route('admins.destroy',$car->id)}}">
        @csrf
        @method('delete')
        <button type="submit" class="btn btn-danger" style="margin: 15px">Delete</button>
    </form>
</div>
<div class="gallery-container">
    ...
    <div class="pagination">
        <!-- Previous Button -->
        @if ($prevCar)
            <a href="{{ route('admins.show', $prevCar->id) }}" class="btn btn-primary" style="margin: 10px">Previous</a>
        @else
        ...
        <!-- Next Button -->
        @if ($nextCar)
            <a href="{{ route('admins.show', $nextCar->id) }}" class="btn btn-primary" style="margin: 10px">Next</a>
        @else
